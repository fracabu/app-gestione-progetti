...
const Sidebar = () => {
  const { activeSection, setActiveSection, sidebarCollapsed, setSidebarCollapsed } = useNavigation();
  ...
  const navigationItems = [
    ...
    { icon: Bell, label: 'Notifiche', id: 'notifications' as const },
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className={`hidden lg:flex ${sidebarCollapsed ? 'w-16' : 'w-60'} bg-gray-50 dark:bg-gray-800 h-screen flex-col border-r border-gray-200 dark:border-gray-700 transition-all duration-300`}>
        {/* Header */}
        <div className={`${sidebarCollapsed ? 'p-3' : 'p-6'} border-b border-gray-200 dark:border-gray-700 transition-all duration-300`}>
          ...
          <nav className={`space-y-1 ${sidebarCollapsed ? 'px-2' : 'px-3'}`}>
            {navigationItems.map((item, index) => (
              <button
                ...
                className={`group flex items-center w-full ${sidebarCollapsed ? 'justify-center px-2 py-3' : 'px-3 py-3'} text-sm font-medium rounded-lg transition-all duration-150 ${
